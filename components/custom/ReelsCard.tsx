...
export function ReelsCard({ videoSrc, isCenter }: ReelsCardProps) {
    ...
    return (
        <div className="flex justify-center items-center transition-all duration-500">
            <div
                className={`relative overflow-hidden rounded-[32px] bg-black border-[3px] transition-all duration-500
                ${isCenter ? "border-[#BF9C66] scale-100" : "border-transparent scale-95 opacity-70"}`}
                style={{ width: '306px', height: '544px' }}
            >
                ...
                <div className="absolute bottom-6 left-0 w-full px-6 flex items-center justify-center gap-2">
                    ...
                    >
                        {isPlaying ? <FaPause size={12} className="text-[#0B2018]" /> : <FaPlay size={12} className="text-[#0B2018]" />}
                    </button>
                </div>
            </div>
        </div>
